# coding: utf8
# Первый полет
# Демонстрация движения астероидов
import random, math
...
class Game(object):
	...
	def play(self):
		...
		games.music.play()
		#загруска и значение фоновой картинки
...
class Wrapper(games.Sprite):
	"""Огибатель. Спрайт который двигаясь огибает граф экран"""
	def update(self):
		"""Переносит спрайт на противоположную сторону"""
...
class Collider(Wrapper):
	"""Погибатель.Огибатель который сталкивается с др обьектами и гибнет"""
	def update(self):
		"""Проверка спрайтов визуально перекрывающи"""
    ...
    def die(self):
		...
		if self.size != Asteroid.SMALL:
			for i in range(Asteroid.SPAWN):
				new_asteriod = Asteroid(game=self.game,
										x=self.x,
										y=self.y,
										size=self.size-1)
				games.screen.add(new_asteriod)
		if Asteroid.total ==0:
			self.game.advance()
...
class Ship(Collider):
	...
	def __init__(self, game, x, y):
		...
		self.game=game
	def update(self):
		"""Вращение корабля при нажатии клавишь"""
		super(Ship, self).update()
		if games.keyboard.is_pressed(games.K_LEFT):
			self.angle-=Ship.ROTETION_STEP
		if games.keyboard.is_pressed(games.K_RIGHT):
			self.angle+=Ship.ROTETION_STEP
		#корабль совершает рывок
		if games.keyboard.is_pressed(games.K_UP):
			Ship.sound.play()
		#изменение гориз и верт скорости корабля
